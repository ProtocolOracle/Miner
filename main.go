...
var (
	infuraURL       = "https://avalanche.drpc.org"
	privateKey      string
...
func init() {
	flag.StringVar(&privateKey, "privateKey", "", "Private key for the account")
	flag.StringVar(&contractAddress, "contractAddress", "0x6C0d13E625bf810fD7ea57ae09F9D4105E8a2C4E", "Address of the contract")
...
func main() {
	...
	_____         _____          ____         _____    ____             ______   
 ...
 |     |    |    ||    |\    \ |    .--.    ||    |   ____ |    | |    ||     /___/|  
 |\     \  /    /||    | |    ||    |  |    ||    |  |    ||    | |    ||     \____|\ 
 | \_____\/____/ ||____| |____||____|  |____||\ ___\/    /||____|/____/||____ '     /|
  \ |    ||    | /|    | |    ||    |  |    || |   /____/ ||    |     |||    /_____/ |
   \|____||____|/ |____| |____||____|  |____| \|___|    | /|____|_____|/|____|     | /
	  \(    )/      \(     )/    \(      )/     \( |____|/   \(    )/     \( |_____|/ 
	   '    '        '     '      '      '       '   )/       '    '       '    )/    
													 '                          '    
	`
	...
	case nonce := <-resultChan:
		...
		etherInWei := new(big.Float).Mul(big.NewFloat(0.01), big.NewFloat(params.Ether))
		etherInWei.Int(weiValue)
		fmt.Print(weiValue)
		auth.Value = weiValue
